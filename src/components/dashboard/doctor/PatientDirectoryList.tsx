...
interface PatientDirectoryListProps {
  patients: Patient[];
}

export function PatientDirectoryList({ patients }: PatientDirectoryListProps) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      <header id="overview" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "1rem" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
          ...
          <button disabled className="btn" style={{ padding: "0.5rem 1.5rem", borderRadius: "99px", background: "var(--accent-aqua)", color: "#000", fontWeight: 600, border: "none", whiteSpace: "nowrap", opacity: 0.5, cursor: "not-allowed" }}>
            + Add Patient
          </button>
        </div>
      </header>
              ...
              {filteredPatients.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ padding: "2rem", textAlign: "center", color: "var(--text-muted)" }}>
                    {searchQuery ? "No patients found matching your search." : "No patients found. Patients will appear here once they book an appointment with you."}
                  </td>
                </tr>
              )}
